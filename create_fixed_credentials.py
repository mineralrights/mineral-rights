...
def create_fixed_credentials():
    """Create properly formatted credentials"""
    print("🔧 Creating fixed credentials...")
    
    try:
        # Decode the original credentials
        ...
        # Fix the private key by adding the missing ending
        if not private_key.endswith('-----END PRIVATE KEY-----'):
            print("🔧 Adding missing -----END PRIVATE KEY-----")
        ...
        print(f"❌ Error creating fixed credentials: {e}")
        import traceback
        traceback.print_exc()
        return None

if __name__ == "__main__":
    fixed_credentials = create_fixed_credentials()
    if fixed_credentials:
        print("\n🎉 Fixed credentials created successfully!")
        print("\n📋 Copy this to your Vercel environment variables:")
        print(f"GOOGLE_CREDENTIALS_BASE64={fixed_credentials}")
    else:
        print("\n💥 Failed to create fixed credentials")
