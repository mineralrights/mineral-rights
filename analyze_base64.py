#!/usr/bin/env python3
"""
Analyze the base64 encoding issue
"""
import base64
import json

GOOGLE_CREDENTIALS_BASE64 = ""

def analyze_base64():
    """Analyze the base64 encoding"""
    print("🔍 Analyzing base64 encoding...")
    
    try:
        # Decode the base64
        credentials_json = base64.b64decode(GOOGLE_CREDENTIALS_BASE64).decode('utf-8')
        print(f"✅ Base64 decoded successfully, length: {len(credentials_json)}")
        
        # Parse JSON
        credentials_info = json.loads(credentials_json)
        print(f"✅ JSON parsed successfully")
        
        # Check the private key
        private_key = credentials_info.get('private_key', '')
        print(f"🔑 Private key length: {len(private_key)}")
        
        # Check if it contains escaped newlines
        if '\\n' in private_key:
            print("⚠️  Private key contains \\n (escaped newlines)")
            print("🔧 This is likely the issue - trying to fix...")
            
            # Try to fix by replacing escaped newlines
            fixed_key = private_key.replace('\\n', '\n')
            print(f"🔧 Fixed key length: {len(fixed_key)}")
            
            # Test the fixed key
            try:
                from cryptography.hazmat.primitives import serialization
                key_bytes = fixed_key.encode('utf-8')
                private_key_obj = serialization.load_pem_private_key(
                    key_bytes,
                    password=None
                )
                print("✅ Fixed private key parsed successfully!")
                
                # Update the credentials
                credentials_info['private_key'] = fixed_key
                
                # Test with GCS
                from google.oauth2 import service_account
                from google.cloud import storage
                
                credentials = service_account.Credentials.from_service_account_info(credentials_info)
                client = storage.Client(credentials=credentials)
                
                bucket_name = "mineral-rights-pdfs-1759435410"
                bucket = client.bucket(bucket_name)
                
                if bucket.exists():
                    print(f"✅ Bucket {bucket_name} exists and is accessible")
                    
                    # Test creating a signed URL
                    from datetime import datetime, timedelta
                    blob_name = f"test-uploads/test-{datetime.now().strftime('%Y%m%d-%H%M%S')}.pdf"
                    blob = bucket.blob(blob_name)
                    
                    expiration = datetime.utcnow() + timedelta(hours=1)
                    signed_url = blob.generate_signed_url(
                        expiration=expiration,
                        method="PUT",
                        content_type="application/pdf"
                    )
                    
                    print(f"✅ Signed URL generated successfully")
                    print(f"🔗 URL: {signed_url[:100]}...")
                    
                    # Generate the corrected base64
                    fixed_json = json.dumps(credentials_info, separators=(',', ':'))
                    fixed_base64 = base64.b64encode(fixed_json.encode('utf-8')).decode('utf-8')
                    
                    print(f"\n🎉 SUCCESS! Here's the corrected GOOGLE_CREDENTIALS_BASE64:")
                    print(fixed_base64)
                    
                    return True
                else:
                    print(f"❌ Bucket {bucket_name} does not exist or is not accessible")
                    return False
                    
            except Exception as e:
                print(f"❌ Fixed key still doesn't work: {e}")
                return False
        else:
            print("✅ Private key does not contain escaped newlines")
            print("🔍 Let's check the key format more carefully...")
            
            # Check the key format
            lines = private_key.split('\n')
            print(f"🔍 Key has {len(lines)} lines")
            print(f"🔍 First line: {lines[0]}")
            print(f"🔍 Last line: {lines[-1]}")
            
            # Check for common issues
            if not private_key.startswith('-----BEGIN PRIVATE KEY-----'):
                print("❌ Key does not start with -----BEGIN PRIVATE KEY-----")
                return False
                
            if not private_key.endswith('-----END PRIVATE KEY-----'):
                print("❌ Key does not end with -----END PRIVATE KEY-----")
                return False
                
            print("✅ Key format looks correct")
            return False
            
    except Exception as e:
        print(f"❌ Error analyzing base64: {e}")
        return False

if __name__ == "__main__":
    success = analyze_base64()
    if success:
        print("\n🎉 Base64 analysis and fix PASSED")
    else:
        print("\n💥 Base64 analysis FAILED")
